...
function university_notice() {
	global $current_user;
	$user_id = $current_user->ID;
	if (!get_user_meta($user_id, 'university_notice_ignore')) {
		echo '<div class="updated notice"><p>'. esc_html__('Thanks for installing University theme!', 'university') .' <a href="https://www.vivathemes.com/" target="blank">'. esc_html__('Check out our other themes  &#8594;', 'university') .'</a><a class="notice-dismiss" href="?university-ignore-notice"><span class="screen-reader-text">Dismiss Notice</span></a></p></div>';
	}
}
add_action('admin_notices', 'university_notice');

function university_notice_ignore() {
	global $current_user;
	$user_id = $current_user->ID;
	if (isset($_GET['university-ignore-notice'])) {
		add_user_meta($user_id, 'university_notice_ignore', 'true', true);
	}
}
add_action('admin_init', 'university_notice_ignore');

add_action('admin_head', 'university_admin_style');
function university_admin_style() {
  echo '<style>
